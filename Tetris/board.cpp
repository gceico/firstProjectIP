...
void Board::updateFile() {
	...
	ifstream fin("Resources/score.txt");
	fin.close();
	ofstream ofs("Resources/score.txt", ios::out | ios::trunc); //delete all data
	for (i = 0; i < TOP_SCORES; i++) {
...
//Check if its possible movement
bool Board::IsPossibleMovement(int pX, int pY, int pPiece, int pRotation) {
	for (int i1 = pX, i2 = 0; i1 < pX + PIECE_BLOCKS; i1++; i2++) {
		for (int j1 = pY, j2 = 0; j1 < pY + PIECE_BLOCKS; j1++, j2++) {
			if (i1<0 || i1>BOARD_WIDTH - 1 || j1 > BOARD_HEIGHT - 1) { // outside limits
				if (mPieces->GetBlockType(pPiece, pRotation, j2, i2) != 0)
					return 0;
			}
